...
mongoose.connect(MONGODB_URI, {
  ...
  console.error('❌ MongoDB خطا:', err.message);
});
...
bot.on('message', async (msg) => {
  if (msg.text === '/start') return; // Skip /start command
  ...
  try {
    ...
    if (user && user.status === 'awaiting_order') {
      ...
      // پاسخ کاربر
      bot.sendMessage(chatId,
        `✅ سفارش شما با شماره #${String(order._id).slice(-8)} ثبت شد!\n\n` +
        `منتظر پاسخ باشید.`
      );
      ...
      ADMIN_CHAT_IDS.forEach(adminId => {
        if (adminId.trim()) {
          bot.sendMessage(adminId.trim(),
            `🆕 سفارش جدید!\n\n` +
            `سفارش: #${String(order._id).slice(-8)}\n` +
